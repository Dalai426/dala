...
export default function CareMove(props: any) {
          ...
          onChange={(e) => {
            setCareData((prevState: any) => ({
              ...
              detail: {
                ...
                name: care_data?.transport_type
                  ? care_data.transport_type[e.target.value]?.name
                  : undefined,
              },
            }));
          }}
        >
          {care_data?.transport_type &&
            Object.keys(care_data.transport_type).map(
              (key: string, index: number) => (
                <option key={index} value={key}>
                  {care_data?.transport_type
                    ? care_data.transport_type[index + 1]?.name
                    : undefined}
                </option>
              )
            )}
